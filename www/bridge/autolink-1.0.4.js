...
Hilite.hiliteElement = function(elm) {
	...
	for (var i = 0; i < Hilite.keywords.length; i ++) {
		...
		text = '\\b'+tmp+'(?=[\., \--";/])';
		//text = '\\b'+tmp+'\\b';
		if (/[ \-]/.test(tmp)) qre2.push(text);
		else qre1.push(text);
	}
	qre = new RegExp(qre.join("|"), "i");
	qre1 = new RegExp(qre1.join("|"), "i");
	qre2 = new RegExp(qre2.join("|"), "i");
	var textproc = function(node) {
		var match2 = qre2.exec(node.data);
		var match1 = qre1.exec(node.data);
		if (match1 || match2)
		{
			var match = match2 ? match2 : match1;
			if (match1 && match2 && match1.index<match2.index) match=match1;
			var val = match[0];
...
Hilite.walkElements = function(node, depth, textproc) {
	var skipre = /^(script|style|textarea|input|select|option|a|h\d)/i;
	var count = 0;
	while (node && depth > 0) {
		count ++;
